...
public class WeaponScript : MonoBehaviour
{
    ...
    void Fire()
    {
        ...
        {
            #region decal-region
            if (weaponSettings.decal && weaponSettings.hitEffect)// IF THE HIT-OBJ IS STATIC
            {
                ...
                GameObject hitEffectGO = Instantiate(weaponSettings.hitEffect, gun_hitInfo.point, lookRot) as GameObject;

                decalGO.transform.SetParent(gun_hitInfo.transform);
                hitEffectGO.transform.SetParent(gun_hitInfo.transform);
                Destroy(decalGO, Random.Range(30f, 40f));
                Destroy(hitEffectGO, Random.Range(30f, 40f));
            }
            #endregion
        }
