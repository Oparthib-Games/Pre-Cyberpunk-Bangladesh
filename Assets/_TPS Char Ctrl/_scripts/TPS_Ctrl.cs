...
namespace sinn.TPS_CTRL
{
    ...
    public class TPS_Ctrl : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            original_capsule_height = capsule_col.height;
            original_capsule_center = capsule_col.center;
        ...
        void ScaleCapsuleForCrouching(bool crouch)
        {
            if(isGrounded && crouch)
            {
                ...
                capsule_col.height /= 2f;
                capsule_col.center /= 2f;
                isCrouching = true;
            }
            else // [PREVENTS PLAYER TO STAND UP WHEN IN THE CROUCH ZONE]
            {
                Ray crouchRay = new Ray(RB.position + Vector3.up * capsule_col.radius * k_Half, Vector3.up);
                float crouchRayLength = original_capsule_height - capsule_col.radius * k_Half;
                if(Physics.SphereCast(crouchRay, capsule_col.radius * 0.5f, crouchRayLength, Physics.AllLayers, QueryTriggerInteraction.Ignore))
                {
                    isCrouching = true; // NOT PRESSING "c" & in crouch zone. this will prevent from standing up
                    return;
                }

                capsule_col.height = original_capsule_height;
                capsule_col.center = original_capsule_center;
                isCrouching = false;
            }
        }
        void PreventStandingInLowHeadroom() // [-KI KORLAM BUJHSI, KEN KORLAM BUJHI NAI-]<<<<<<<<<<<<>>>>>>>>>>>>
        {
            ...
            if (!isCrouching)
            {
                Ray crouchRay = new Ray(RB.position + Vector3.up * capsule_col.radius * k_Half, Vector3.up);
                float crouchRayLength = original_capsule_height - capsule_col.radius * k_Half;
                if (Physics.SphereCast(crouchRay, capsule_col.radius * k_Half, crouchRayLength, Physics.AllLayers, QueryTriggerInteraction.Ignore))
                {
                    isCrouching = true;
                }
            }
